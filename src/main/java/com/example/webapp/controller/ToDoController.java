...
public class ToDoController {
     ...
     */
    @PostMapping("/save")
    public String create(@Validated @ModelAttribute ToDoForm form, // @Validated と @ModelAttribute を追加
        ...
        if (bindingResult.hasErrors()) {
            // 新規登録画面の設定
            form.setIsNew(true); // 新規登録なのでisNewはtrue
            return "todo/form";
        }
        // エンティティへの変換
     ...
     */
    @PostMapping("/update")
    public String update(@Validated @ModelAttribute ToDoForm form, // @Validated と @ModelAttribute を追加
                         BindingResult bindingResult, // ★追加
                         RedirectAttributes attributes) {
        // === バリデーションチェック ===
        // 入力チェックNG：入力画面を表示する
        if (bindingResult.hasErrors()) {
            // 更新画面の設定
            form.setIsNew(false); // 更新なのでisNewはfalse
            return "todo/form";
        }
        // エンティティへの変換
        ToDo ToDo = ToDoHelper.convertToDo(form);
